...
class Terrain:
    def __init__(self, scale, SCREEN_WIDTH, SCREEN_HEIGHT):
        ...
        # Track chunk positions as floats to avoid precision issues
        self.chunk_positions = []
        
        for i in range(2):
            chunk_rect = pygame.Rect(i * SCREEN_WIDTH, 0, SCREEN_WIDTH, SCREEN_HEIGHT)
            self.chunks.append(chunk_rect)
            self.chunk_positions.append(float(i * SCREEN_WIDTH))
    ...
    def terrain_loop(self):
        ...
        for i, chunk in enumerate(self.chunks):
            # Update rect position from stored float position
            chunk.x = int(self.chunk_positions[i])

            if chunk.right < 0:
                furthest_right = max(self.chunks, key=lambda r: r.right)
                self.chunk_positions[i] = float(furthest_right.right)
                chunk.x = int(self.chunk_positions[i])
    ...
    def terrain_movement(self, dt, speed):
        ...
        for i in range(len(self.chunks)):
            # Update position as float for precision
            self.chunk_positions[i] -= (speed * self.movement_speed * dt)
            # Apply integer position to rect
            self.chunks[i].x = int(self.chunk_positions[i])